...
const Search = () => {
  ...
  const handleFormSubmit = (event) => {
    ...
    router.push(`${pathName}?${router.pathname}?${params.toString()}`);
  };
  ...
  return (
    <div className="row-start-2 col-span-2 border-0 md:border flex shadow-sm hover:shadow-md transition-all md:rounded-full items-center px-2">
      <div className="grid md:grid-cols-3 lg:grid-cols-7 gap-4 divide-x py-2 md:px-2 flex-grow">
        <form onSubmit={handleFormSubmit} className=" lg:col-span-3">
          <input
            type="text"
            placeholder="Where to?"
            className="px-3 bg-transparent focus:outline-none  placeholder:text-sm"
            value={searchTerm}
